...
module CBC
  ...
  def encrypt_stream(plainStream, cryptStream)
    ...
    remainingMessageBytes.upto(block_size()-2) { buffer << rand(256).chr }
  ...
  def encrypt_file(plainFilename, cryptFilename)
    plainFile = carefully_open_file(plainFilename, 'r')
    cryptFile = carefully_open_file(cryptFilename, 'w+')
  ...
  def decrypt_file(cryptFilename, plainFilename)
    cryptFile = carefully_open_file(cryptFilename, 'r')
    plainFile = carefully_open_file(plainFilename, 'w+')
